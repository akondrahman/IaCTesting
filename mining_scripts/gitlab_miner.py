import pymysql, http.client, os, git, shutil,stat
from os import path
import requests, json
#set GIT_PYTHON_GIT_EXECUTABLE=C:\Program Files\Git\cmd\git.exe
db_conn = pymysql.connect("localhost","root","","test", charset='utf8' )
cursor = db_conn.cursor()
github_conn = http.client.HTTPSConnection("api.github.com")

def get_all_gitlab_repo_id():
   
    try:
        cursor.execute('select * from gitlab_repos where no_of_developers>0 and repo_lifetime_in_month>0 and iac_script_percentage is null')
        rows = cursor.fetchall()
    except:
        print(Exception)
        rows = []
       
    return rows


def update_iac_percentage(repo):
    ext_dir = r'C:\mined_repos'
    ansible_percentage = find_iac_percentage(repo[1], ext_dir)
    
    update_query = "update gitlab_repos set  iac_script_percentage = %s where id = %s"
    val = (ansible_percentage, repo[0])
    cursor.execute(update_query, val)
   
    db_conn.commit()



def download_repo(ext_dir, repo_name):
    print(f"starting to download repository {repo_name}")
    print("=================")
    base_repo = repo_name.split("/")[1]
    full_dir = ext_dir+"\\"+base_repo
    
    repo_url = "https://gitlab.com/"+repo_name
    try:
        git.Git(ext_dir).clone(repo_url)
        print("Download completed")
        print("=================")    
        return True
       
    except Exception as e:
        print(e)
        if (os.path.exists(full_dir)):
            return True
        else:
            return False
   
def delete_repo(full_dir):
    print(f"deleting the directory: {full_dir}")
    print("=================")
    for root, dirs, files in os.walk(full_dir):
        for dir in dirs:
            os.chmod(path.join(root, dir), stat.S_IRWXU)
        for file in files:
            os.chmod(path.join(root, file), stat.S_IRWXU)
   
    try:
        shutil.rmtree(full_dir)
        print("deletion completed")
        print("=================")

    except Exception as e:
        print(e)
       
   

def count_yml_file(dir_name ):
    total_file_count = 0
    yml_file_count = 0
    print(f"Counting the yml files in repository {dir_name}")
    print("=================")
    for root_, dirnames, filenames in os.walk(dir_name):
        for file_ in filenames:
            full_path_file = os.path.join(root_, file_)
            if(os.path.exists(full_path_file)):
                total_file_count += 1
                if (file_.endswith("yaml") or file_.endswith("yml"))  :
                    yml_file_count += 1
   
    print("Counting completed")                
    print("=================")    
    return total_file_count, yml_file_count



def find_iac_percentage(repo_name, ext_dir):

    if download_repo(ext_dir, repo_name):
        base_repo = repo_name.split("/")[1]
        full_dir = ext_dir+"\\"+base_repo
        total_files, yml_files = count_yml_file(full_dir)
        if total_files > 0:
            percentage = yml_files/total_files*100
        else:
            percentage = 0
        print(f'Ansible repo percentage maximum {percentage}')
       
       
        delete_repo(full_dir)

       
        return percentage
       

   
ext_dir = r'C:\mined_repos'
repo_name = 'jpbarto/cicd_tf_workshop'
#repo_type = 1
#find_iac_percentage(repo_name, ext_dir, repo_type)
# download_repo(ext_dir, repo_name)

repos = get_all_gitlab_repo_id()

for repo in repos:
    print (f'\n ====Repo ID is {repo[0]}\n\n ====')
    update_iac_percentage(repo)
    
        
